...
/*
** File: coveragetest_MY_app.c
**
** Purpose:
** Coverage Unit Test cases for the MY Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the MY application.
**
...
#include "my_app_coveragetest_common.h"
#include "ut_my_app.h"

/* to get the MY_LIB_Function() declaration */
...
void Test_MY_APP_Main(void)
{
    ...
    /*
     * MY_APP_Main does not return a value,
     ...
     * The first is for MY_APP_Init().  As this is in the same
     ...
     * the MY_APP_Init() failure path was taken.
     */
    MY_APP_Main();
     ...
     */
    UtAssert_True(MY_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "MY_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)MY_APP_Data.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, MY_APP_PIPE_ERR_EID, "MY APP: SB Pipe Read Error, App Will Exit");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "MY_APP_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MY_APP_Init(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(MY_APP_Init(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(MY_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(MY_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UT_TEST_FUNCTION_RC(MY_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 5, "CFE_ES_WriteToSysLog() called");
}

void Test_MY_APP_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void MY_APP_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        MY_APP_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, MY_APP_INVALID_MSGID_ERR_EID, "MY: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = MY_APP_CMD_MID;
    FcnCode   = MY_APP_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    MY_APP_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = MY_APP_SEND_HK_MID;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    MY_APP_ProcessCommandPacket(&TestMsg.SBBuf);
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "MY_APP_COMMAND_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MY_APP_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void MY_APP_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        MY_APP_NoopCmd_t          Noop;
        MY_APP_ResetCountersCmd_t Reset;
        MY_APP_ProcessCmd_t       Process;
    } TestMsg;
    UT_CheckEvent_t EventTest;
    ...
    UT_CheckEvent_Setup(&EventTest, MY_APP_COMMANDNOP_INF_EID, NULL);

    MY_APP_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_True(EventTest.MatchCount == 1, "MY_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = MY_APP_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, MY_APP_COMMANDRST_INF_EID, NULL);

    MY_APP_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_True(EventTest.MatchCount == 1, "MY_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of PROCESS */
    /* note this will end up calling MY_APP_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = MY_APP_PROCESS_CC;
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, MY_APP_COMMAND_ERR_EID, "Invalid ground command code: CC = %d");
    MY_APP_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_MY_APP_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void MY_APP_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(MY_APP_SEND_HK_MID);

    /* Set message id to return so MY_APP_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
...
void Test_MY_APP_NoopCmd(void)
{
    /*
     * Test Case For:
     * void MY_APP_NoopCmd( const MY_APP_Noop_t *Msg )
     */
    MY_APP_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;
    ...
    UT_TEST_FUNCTION_RC(MY_APP_Noop(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MY_APP_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MY_APP_ResetCounters(void)
{
    /*
     * Test Case For:
     * void MY_APP_ResetCounters( const MY_APP_ResetCounters_t *Msg )
     */
    MY_APP_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CheckEvent_Setup(&EventTest, MY_APP_COMMANDRST_INF_EID, "MY: RESET command");

    UT_TEST_FUNCTION_RC(MY_APP_ResetCounters(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MY_APP_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MY_APP_ProcessCC(void)
{
    /*
     * Test Case For:
     * void  MY_APP_ProcessCC( const MY_APP_Process_t *Msg )
     */
    MY_APP_ProcessCmd_t TestMsg;
    MY_APP_Table_t      TestTblData;
    void *                  TblPtr = &TestTblData;

    memset(&TestTblData, 0, sizeof(TestTblData));
    memset(&TestMsg, 0, sizeof(TestMsg));

    /* Provide some table data for the MY_APP_Process() function to use */
    TestTblData.Int1 = 40;
    TestTblData.Int2 = 50;
    UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
    UT_TEST_FUNCTION_RC(MY_APP_Process(&TestMsg), CFE_SUCCESS);
    ...
    /*
     * Confirm that the MY_LIB_Function() call was done
     * NOTE: This stub is provided by the MY_lib library
     */
    UtAssert_True(UT_GetStubCount(UT_KEY(MY_LIB_Function)) == 1, "MY_LIB_Function() called");
     ...
     */
    UT_SetDefaultReturnValue(UT_KEY(CFE_TBL_GetAddress), CFE_TBL_ERR_UNREGISTERED);
    UT_TEST_FUNCTION_RC(MY_APP_Process(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
}

void Test_MY_APP_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool MY_APP_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, MY_APP_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    MY_APP_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "MY_APP_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    MY_APP_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MY_APP_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MY_APP_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(MY_APP_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return MY_APP_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + MY_APP_TBL_ELEMENT_1_MAX;
    UT_TEST_FUNCTION_RC(MY_APP_TblValidationFunc(&TestTblData), MY_APP_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_MY_APP_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(MY_APP_Main);
    ADD_TEST(MY_APP_Init);
    ADD_TEST(MY_APP_ProcessCommandPacket);
    ADD_TEST(MY_APP_ProcessGroundCommand);
    ADD_TEST(MY_APP_ReportHousekeeping);
    ADD_TEST(MY_APP_NoopCmd);
    ADD_TEST(MY_APP_ResetCounters);
    ADD_TEST(MY_APP_ProcessCC);
    ADD_TEST(MY_APP_VerifyCmdLength);
    ADD_TEST(MY_APP_TblValidationFunc);
    ADD_TEST(MY_APP_GetCrc);
}
